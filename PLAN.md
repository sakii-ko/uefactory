...
> **当前主线:T1.1(4090 节点上线)→ T1.2(MRQ headless spike)→ T1.3 JobSpec → T1.4 多通道
> → T1.5 光照预设 → T1.6 统一执行器 + contact sheet + turntable 视频 → T1.7 收尾 → M1 验收。**
...
### T1.1 4090 节点上线 `#remote`(M0 顺延项,首任务)
- [ ] `uef node init 4090` + `uef node provision 4090`(tmux + `--partial`;落点 `/home/lyf/uef/engine/`);
  ssh 时通时断已实证——provision 前探测,失败则**指数退避重试**(30s/2min/10min),连续失败 >2h 记录证据并发 BLOCKED 信号;
- [ ] 共享机器纪律:传输/解压前检查剩余空间(`/home` 仅 ~209G,我们总占用硬上限 150GB),不影响他人进程;
- [ ] `uef doctor --host 4090` 通过(存储 WARN 允许)→ `uef render smoke --host 4090` 拉回的图过校验;
- [ ] 传输耗时/实测带宽/GPU 挑选策略(8 卡,选空闲卡:`CUDA_VISIBLE_DEVICES` 或 UE 的 GPU 选择参数,实测记录)写 WORKLOG。
- **DoD**:4090 冒烟图过校验 + 确定性(两跑一致);渲染期间不影响该机其他用户(nvidia-smi 前后对照留证)。
- 提示:provision 传输在远端 tmux 挂上之后的等待时间,可提前开始 T1.2 的 spike(唯一允许的并行)。
...
- **DoD**:同一 job 在本地 + l40s + 4090 三处跑通,产物结构一致;contact sheet + turntable 视频可看(M1 验收演示物)。
...
**任务顺序**:严格 T1.1 → T1.2 → … → T1.7 串行(唯一例外:T1.1 的 provision 传输挂后台期间可开始 T1.2);
每任务完成即在 WORKLOG 登记并可发 INFO 信号,**中途发现 PLAN 未覆盖的决策点一律 BLOCKED + QUESTIONS**。
分支:`feat/m1-render`(从 main 切)。预计 4–7 个工作日(MRQ spike 与 4090 网络为主要不确定项)。
