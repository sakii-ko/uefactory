...
> ✅ Review #3:`docs/reviews/2026-07-08-interim-3-t03-done.md` —— T0.3 复审通过,Planner 独立
> 实跑复现,确定性渲染(mean_luma 逐位一致)自此为必须保持的不变量。
> **当前主线:F5 → F6 → F7(逐个小 commit,见 review #1)→ T0.5 收尾(F8 + review #3 两个 NIT)
> → `REVIEW REQUESTED`。** T0.6/T0.7 仍冻结,正式 review 合入 main 后解冻。
