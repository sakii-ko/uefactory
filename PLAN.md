...
> 最后更新:2026-07-08(第 3 次) · 当前阶段:**M0(骨架与冒烟渲染)** · 状态:T0.1/T0.2/T0.4 已交付;**T0.3 REOPENED**
> ⚠️ Review #2:`docs/reviews/2026-07-08-interim-2-t03-reopened.md` ——
> **T0.3 产物是 Canvas 2D 图案而非场景渲染,验收对象被替换,重开。**剩余工作 = 修 SceneCapture
> 近黑(自动曝光/灯光 mobility/预热,详见 review §排查顺序),产出真场景图。F3/F4 已确认修好。
> **规则新增:DoD 的验收对象不可自行替换;做不到就标 BLOCKED 并写 QUESTIONS,等 Planner 裁定。**
> T0.6/T0.7 维持冻结。节奏:一次只推进一个任务;宁可批次小,不要质量腐化。
...
**任务顺序(2026-07-08 修订)**:当前唯一主线 = 中期 review 修正(F1 → F2/F3)→ 完成 T0.3 → F4–F7 逐个小 commit → T0.5 收尾(含 F8)→ 请求正式 review。**T0.6/T0.7 冻结**,待正式 review 通过、Planner 在本文件解冻后再开工。预计 2~3 个工作日。
