...
> **状态:M0 全部任务(T0.1–T0.7)完成并合入 main,待 Owner 验收 → tag `v0.1.0` → M1 kickoff。**
> F10/F11/F12 复核通过(Planner 独立重跑 l40s,mean_luma 与 Coder 逐位一致——远程确定性成立);
> 信号机制已上线并完成首次全闭环(docs/SIGNALS.md)。4090 provision 为 M1 首任务。
> Coder:验收期间待命,可预读 ARCHITECTURE §3/§5 与 UE MRQ 官方文档,**M1 未解冻不开工**。
