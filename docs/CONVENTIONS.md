...
> 对当前执行代理有强制力。Review 时按本文逐条对照;不符合即修复后重验。
> 会改变项目目标或扩大权限的规范问题登记到 `docs/QUESTIONS.md`,由 Owner 拍板。
...
正确动作是继续排查和安全替代;确需改变验收目标或外部条件时才标 BLOCKED、写入 QUESTIONS 并请 Owner 裁定。
每个任务的 DoD 至少包含:
...
**身份**:使用本 checkout 已配置的项目身份,提交前核对 `git config user.name/user.email`;
不再用作者字段模拟 Planner/Coder 分工。历史提交的身份和 trailer 不改写,禁止为统一外观重写历史。

**分支**:
- `main`:只有在正式 review 与全量验收通过后才合入(`merge --no-ff`)。
...
**tag**:里程碑验收后打 `vX.Y.0`;中间可打 `m0-t3-done` 类轻量 tag 标记任务节点。
...
- **每次 commit 后立即 push 当前分支**(`git push origin HEAD`,分支首推加 `-u`)——远端即备份,
  也是正式 review 的依据;请求 review 时远端必须与本地一致(WORKLOG 里的 sha 以远端为准)。
- push 失败(网络等)不阻塞继续开发,但要在 WORKLOG 标注并尽快补推。
- `main` 与 tag 仅在里程碑 review 通过后 push;**禁止 force-push 任何已共享分支**。

## 6. 目录结构(固定,新目录需 Planner 批准)

```
uefactory/
├── PLAN.md                  # 执行代理维护:计划与当前 Sprint
├── README.md
├── pyproject.toml
