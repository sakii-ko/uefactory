...
```text
 ...
 uef CLI ── JSON job + UEF_JOB_FILE ──► UE 5.5.4 / AssetImportTask
                          ...
                          ┌────────────────────────┼─────────────────────┐
                          ▼                        ▼                     ▼
                 /Game/UEF/Ingested/       out/ manifests        SQLite catalog v5
                          │                        │                     │
                          └──────── catalog asset/scene resolver ───────┘
                                      ...
                                      out/ frames + visual reports
                                                   │
                                                   ▼
                              verified stage showcase MP4 + manifest

 Poly Haven HDRI/PBR ── listing-bound resource sync ──► immutable revision roots
                                                           │
                                                           ▼
                                                  strict CPU validation
                                                           │
                                                           ▼
                                                atomic resource cohort publish
...
当前九张业务表分成 asset/scene 与 resource 两套 typed lineage，不把 HDRI/PBR 强塞进 StaticMesh
资产行：
...
| `resources` | HDRI/PBR identity、provider revision、profile/resolution、license、状态与 cohort digest |
| `resource_files` | source file role、path/URL、bytes/hash、颜色空间、normal convention、channels、尺寸 |
| `resource_artifacts` | source manifest、CPU validation manifest、PBR material descriptor 等绑定证据 |
| `resource_bindings` | resource 到 asset、scene 或另一个 consumer resource 的唯一 role binding |

模型状态是 `raw | imported | render_ok | failed`；场景状态是
`raw | built | render_ok | failed | quarantined`；resource 状态是
`verified | ready | failed | quarantined`。license、tier、状态组合、lower_snake_case id、
repo-relative path、UE package namespace、hash 格式和状态所需字段均同时受 Python 校验与 SQL
CHECK/FK 约束。`finalize_import`、`finalize_render`、scene generation replacement 与
`finalize_resource` 都在单个 SQLite write transaction 内更新 parent record 和 evidence cohort，
避免半提交 catalog。

schema v4 首次加入四张 resource 表；schema v5 在 `resources` 增加 append-only
`published_once`。迁移会把 v4 的 `verified/ready` 行标为已发布；对已经变成
`failed/quarantined` 的旧行，只有 bundle/content digest、唯一 primary file 和精确绑定 active digest
的 `resource_source_manifest` 同时存在时才恢复 publication lineage。SQL trigger 禁止把
`published_once=1` 改回 0，也禁止创建没有 publication lineage 的 `verified/ready` 行。

普通 `upsert_resource` 不得写 `verified/ready`；这两个状态只能通过 `finalize_resource` 携带完整
files/artifacts cohort 原子发布。发布后 resource metadata、file bytes/path/hash 与已有 artifact evidence
不可变；`verified -> ready` 只允许保留原 files/evidence 并追加该 profile 要求的 validation proofs。
同一 evidence 的精确 replay 是幂等操作，metadata drift、缺/多 primary file、artifact 未绑定当前
resource/digest、非法状态回退或已发布 cohort 变更都会 fail closed。

### 5.1 Poly Haven resource sync 数据流

`uef acquire polyhaven-resources` 的 `--source-id` 仍必须出现在当前 official listing 中，不能直接用
files URL 绕过 provider revision、metadata 和许可发现。HDRI 与 PBR 共用 Poly Haven source lock、
rate/retry/quota ledger，但分别维护 state、failure journal 和 run manifests：

```text
official /assets listing
  -> validate listing + bind source id/current files revision
  -> official /files payload + HTTPS allowlist/runtime quota
  -> immutable data/acquire/polyhaven/resources/<kind>/<source>/<revision>/<profile>/<resolution>/
  -> provider MD5 + local SHA-256 + bundle/content digest
  -> strict CPU HDR or PNG/PBR cohort validation
  -> out/resources/polyhaven/<resource_id>/ source/validation/descriptor evidence
  -> Catalog.finalize_resource(files + artifacts) in one transaction
  -> durable state/run receipt/failure journal reconciliation
```

HDRI `radiance_hdr_v1` 必须是完整可解码的 linear Radiance HDR、正尺寸 2:1，并生成
`resource_source_manifest + hdri_validation_manifest`。PBR `ue_pbr_png_v1` 必须是同尺寸 PNG cohort：
sRGB base color、DirectX normal，以及 data-space roughness 或 packed ARM；当前 Poly Haven adapter 固定
Diffuse + `nor_dx` + ARM，并生成 source manifest、material descriptor 与 validation manifest。

ready cohort 每次重放都会重新验证 source files、artifact files、hash 和语义。catalog 已提交但 HDRI
兼容 alias 尚未完成时使用 durable intent 恢复；`data/hdri/<source>_<resolution>.hdr` 是独立 regular-file
兼容副本，不以 hardlink 假装独立 publication。catalog、state 或 run receipt 缺失时只从仍可精确验真的
immutable bytes/evidence 恢复，不回滚到较旧 provider revision。
...
### 7.1 Stage showcase 归档

`uef render showcase <render-run-dir> --stage <slug>` 是已验证 render evidence 到可直接播放阶段样例的
单向发布器，不是任意图片转码器。source 必须在 repo 内、manifest 必须是成功的 schema v3 job，且
`beauty_lit/frame_0000.png` 与 `object_mask/frame_0000.exr` 起始的两组帧序列连续、逐帧对应、至少
72 帧、偶数分辨率、短边至少 1080。发布器重新解码 beauty/mask 并与 render manifest 的 pixel
SHA-256 对账；mask 每帧要求 foreground area >=10%、bbox area >=18%、最小 frame margin >=3%。
这会在编码前拒绝主体过小或裁切的规格合格假样片；编码前后还会再次确认 source manifest 与两组
frame bytes 未变化。

编码固定为 24 fps、H.264 `libx264`、CRF 16、slow preset、`yuv420p`、无音频与 `+faststart`。
随后用 ffprobe 精确复核唯一 video stream、codec、分辨率、帧率、帧数、时长和文件大小，并检查 MP4
faststart，再把目录从 staging 原子 rename 到：

```text
out/showcases/<stage>/<UTC timestamp>_<asset slug>/
  showcase.mp4
  manifest.json
  source_render_manifest.json
```

showcase manifest 保存 source manifest path/hash 与归档副本、beauty/mask 逐帧 file/pixel hash、mask
coverage/bbox/margin 汇总、video hash/probe、license/attribution、git commit/dirty 状态和 encoding
command digest。只有已经通过 render gate 的数据能成为 showcase；failed/quarantined 候选不能替代
缺失类别。某一阶段尚无合格人物数据时，该类别必须明确记为 `not_available`，不能用 scene 中疑似
人物或失败实验伪装成已交付样本。
